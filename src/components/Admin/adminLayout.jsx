...
import { Layout, Menu, Avatar, Flex, Space, message, Input, Badge, Breadcrumb, Dropdown, Card, Tooltip } from 'antd';
    ...
    useEffect(() => {
        ...
        if (pathSegments.length > 1) {
            ...
            pathSegments.slice(1).forEach((segment) => {
                currentPath += `/${segment}`;
                const formattedTitle = segment.split('-').map(word => 
                    word.charAt(0).toUpperCase() + word.slice(1)
                ).join(' ');
                
                breadcrumbArray.push({
                    title: location.pathname === `/admin${currentPath}` 
                        ? formattedTitle
        ...
        <Layout hasSider>
            ...
            <Layout className="site-layout">
                <Header className="admin-header !bg-card-color">
                    <Flex align="center">
                        <Tooltip title={collapsed ? "Expand menu" : "Collapse menu"}>
                            <MenuOutlined className='text-base cursor-pointer hover:text-blue-500 transition-all' onClick={() => setCollapsed(!collapsed)} />
                        </Tooltip>
                ...
                <Content className="admin-content">
                    <Card className="admin-breadcrumb mb-4">
