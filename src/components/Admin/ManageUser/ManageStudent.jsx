...
const ManageStudent = () => {
  ...
  const handleEdit = async (record) => {
    try {
      ...
      message.error(
        t(
          "admin.student.messages.loadError",
  ...
  const handleDelete = async () => {
    Modal.confirm({
      ...
      content: t(
        "admin.student.modal.deleteConfirm.content",
      ...
      onOk: async () => {
        try {
          ...
          message.success(
            t(
              "admin.student.messages.deleteSuccess",
          ...
          message.error(
            t("admin.student.messages.deleteError", "Không thể xóa người dùng")
          );
        } finally {
          setLoading(false);
        }
  ...
  const handleModalOk = async (values) => {
    try {
      ...
      if (response && response.status === "OK") {
        ...
        message.success(
          t(
            "admin.student.messages.updateSuccess",
        ...
        message.error(
          t(
            "admin.student.messages.updateError",
      ...
      message.error(
        t(
          "admin.student.messages.validateError",
    ...
    <>
      ...
      >
        <Form form={form} onFinish={handleModalOk} layout="vertical">
          ...
          <Form.Item
            name="dob"
            label={t("admin.student.modal.form.dob.label", "Ngày sinh")}
          >
            <DatePicker
              format={"DD/MM/YYYY"}
              style={{ width: "100%" }}
          ...
          >
            <Select>
              <Option value={true}>
                {t(
                  "admin.student.modal.form.status.options.active",
                  "Hoạt động"
                )}
              </Option>
              <Option value={false}>
