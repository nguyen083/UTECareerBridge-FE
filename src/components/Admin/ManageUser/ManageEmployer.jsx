  ...
  const handleModalOk = async (values) => {
    try {
      ...
      updateUser(selectedUser, values).then((res) => {
        ...
        } else message.error(t("admin.employer.messages.updateError"));
      });
      handleModalCancel();
    } catch (error) {
      console.log(error);
  ...
  return (
    <>
      <BoxContainer className="shadow-md">
        ...
        <Divider />
        <TableListUser fetch={res} userType="employer" onEdit={handleEdit} />
      </BoxContainer>
      <Modal
        ...
        <Form form={form} onFinish={handleModalOk} layout="vertical">
          ...
          >
            <Select defaultValue={true}>
              <Option value={true}>
                {t("admin.employer.modal.form.status.options.active")}
              </Option>
              <Option value={false}>
                {t("admin.employer.modal.form.status.options.blocked")}
              </Option>
            </Select>
          </Form.Item>
        </Form>
