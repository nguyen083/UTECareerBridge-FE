...
const EventList = ({ isFetching, setIsFetching, eventType }) => {
  ...
  const fetchEvent = () => {
    setLoading(true);
    getAllEvent({ page: currentPage, size: pageSize, eventType: eventType })
      .then((res) => {
        ...
        deleteEvent(item.eventId)
          .then((res) => {
            if (res.status === "OK") {
              ...
              deleteImageFromCloudinaryByLink(item.eventImage).then(() => {
                fetchEvent();
              });
            } else {
                ...
                description={
                  <>
                    <Text className="font-bold">
                      {t("admin.event.list.eventDate")}:{" "}
                    </Text>{" "}
                    <Text>{item.eventDate}</Text>
                    <br />
                    <Text className="font-bold">
                      {t("admin.event.list.location")}:{" "}
                    </Text>{" "}
          ...
          <Flex justify="end" align="center" gap={10}>
            ...
            >
              <Select.Option value="SEMINAR">
                {t("admin.event.form.fields.eventType.options.seminar")}
              </Select.Option>
              <Select.Option value="CONFERENCE">
                {t("admin.event.form.fields.eventType.options.conference")}
              </Select.Option>
              <Select.Option value="WORKSHOP">
                {t("admin.event.form.fields.eventType.options.workshop")}
              </Select.Option>
              <Select.Option value="CAREER_FAIR">
                {t("admin.event.form.fields.eventType.options.careerFair")}
              </Select.Option>
