...
const CVAnalysis = () => {
  ...
  const { data: resumeData, isLoading: isLoadingResumes } = useResume();
  ...
  const formatRelativeTime = (dateString) => {
    ...
    if (diffInSeconds < 60) {
      ...
      return `${minutes} ${
        minutes === 1 ? t("common.minuteAgo") : t("common.minutesAgo")
      }`;
    } else if (diffInSeconds < 86400) {
      const hours = Math.floor(diffInSeconds / 3600);
      return `${hours} ${
        hours === 1 ? t("common.hourAgo") : t("common.hoursAgo")
      }`;
          ...
          recommendations.length > 0
            ? Math.round(
                recommendations.reduce(
                  (sum, job) => sum + job.match_score * 100,
                  0
