...
import { Button, Divider, Typography, Table, message, Flex, Card, Row, Col, Empty, Tooltip, Badge } from 'antd';
import './orderPage.scss';
import BoxContainer from '../../Generate/BoxContainer';
import { CloseOutlined, DeleteOutlined, MinusOutlined, PlusOutlined, ShoppingCartOutlined, TagOutlined } from '@ant-design/icons';
...
const OrderPage = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      dataIndex: 'price',
      key: 'price',
      render: (text) => <span className="package-price">{text.toLocaleString('vi-VN', { style: 'currency', currency: 'VND' })}</span>,
    },
    {
    ...
    <>
      <BoxContainer>
        <div className="cart-page-header">
          <Title level={4}><ShoppingCartOutlined /> {t('employer.orders.shoppingCart')}</Title>
        </div>
              ...
              <Card 
                title={
                  <Flex align="center" gap={8}>
                    ...
                    <Text className='card-title'>{t('employer.orders.packageList')}</Text>
                    <Badge count={cartItems.length} showZero className="cart-badge" />
                  </Flex>
                } 
            ...
            <Col xs={24} md={10}>
              <Card 
                title={
                  <Flex align="center" gap={8}>
                    <TagOutlined />
                    <Text className='card-title'>{t('employer.orders.orderInfo')}</Text>
                  </Flex>
                } 
              ...
              >
                <div className="info-item">
                  <Text className="text-base">{t('employer.orders.subtotal')}</Text>
                ...
                {selectedVoucher && (
                  <div className="info-item discount-row">
                    ...
                    <Text className="text-base font-bold discount-value">- {selectedVoucher?.discount}%</Text>
                  </div>
                )}
                ...
                <div className="voucher-section">
                  ...
                  {selectedVoucher && (
                    <div className="selected-voucher-container">
                      ...
                      />
                      <Text className="selected-voucher">{selectedVoucher.code}</Text>
                    </div>
                  )}
                </div>
                
                <div className="total-section">
                  <Text className="total-label">{t('employer.orders.total')}</Text>
                  <div className="total-value-container">
                    {selectedVoucher && (
                      <Text type='danger' className="original-price" delete>
                        {getTotalPrice().toLocaleString('vi-VN', { style: 'currency', currency: 'VND' })}
                      </Text>
                    )}
                    <Text className="final-price">
                      {getTotalWithTax().toLocaleString('vi-VN', { style: 'currency', currency: 'VND' })}
                    </Text>
                  </div>
                </div>
