...
import { Tabs, Card, Row, Col, Statistic, Button, Typography, Empty } from 'antd';
import TableListJobs from './TableListJobs';
import BoxContainer from '../../Generate/BoxContainer';
import { useTranslation } from 'react-i18next';
import { PlusOutlined, CheckCircleOutlined, ClockCircleOutlined, CloseCircleOutlined, PauseCircleOutlined } from '@ant-design/icons';
...
const ManageListJobs = () => {
    ...
    const StatCard = ({ title, count, icon, color, description }) => (
        <Card className={`stat-card ${activeKey === title ? 'active' : ''}`} onClick={() => setActiveKey(title)}>
            <div className="stat-icon" style={{ backgroundColor: `${color}15` }}>
                {icon}
            </div>
            <div className="stat-content">
                <Statistic value={count} valueStyle={{ color }} />
                <div className="stat-title">{t(`employer.manageJobs.tabs.${title.toLowerCase()}`)}</div>
            ...
            <BoxContainer className='shadow-md job-header'>
                <div className="job-header-content">
                    <div>
                        ...
                        <Text type="secondary">{t('employer.manageJobs.description') || "Manage and track all your job postings"}</Text>
                    </div>
                    <Button 
                        type="primary" 
                        icon={<PlusOutlined />}
                ...
                <Row gutter={[16, 16]}>
                    <Col xs={24} sm={12} md={6}>
                        <StatCard 
                            ...
                            color="#52c41a"
                            description={t('employer.manageJobs.activeDescription') || "Live jobs visible to applicants"}
                        />
                    </Col>
                    <Col xs={24} sm={12} md={6}>
                        <StatCard 
                            ...
                            color="#faad14"
                            description={t('employer.manageJobs.inactiveDescription') || "Hidden jobs not visible to applicants"}
                        />
                    </Col>
                    <Col xs={24} sm={12} md={6}>
                        <StatCard 
                            ...
                            color="#1890ff"
                            description={t('employer.manageJobs.pendingDescription') || "Jobs awaiting approval"}
                        />
                    </Col>
                    <Col xs={24} sm={12} md={6}>
                        <StatCard 
                            ...
                            color="#ff4d4f"
                            description={t('employer.manageJobs.rejectedDescription') || "Jobs that were not approved"}
                        />
                    </Col>
                </Row>
            </BoxContainer>
                ...
                <Tabs 
                    ...
                    tabBarExtraContent={
                        <div className="total-count">
                            <Text>
                                {t('employer.manageJobs.totalJobs')}: <Text strong>{Object.values(statistics).reduce((a, b) => a + b, 0)}</Text>
                            </Text>
                        </div>
                    }
                >
                    ...
                    <Tabs.TabPane
                        tab={
                            <span className="tab-with-count">
                                ...
                                <span className="count-badge inactive">{statistics.inactive}</span>
                            </span>
                        }
                        key="INACTIVE"
                    />
                    <Tabs.TabPane
                        tab={
                            <span className="tab-with-count">
                                ...
                                <span className="count-badge pending">{statistics.pending}</span>
                            </span>
                        }
                        key="PENDING"
                    />
                    <Tabs.TabPane
                        tab={
                            <span className="tab-with-count">
                                ...
                                <span className="count-badge rejected">{statistics.rejected}</span>
                            </span>
                        }
                        key="REJECTED"
                    />
